...
<div class="table-responsive" id="conttabl">
    <table class="table table-condensed   table-bordered" id="tblcapt">
        ...
        foreach ($recordkey as $row) {
            ...
            foreach ($card as $row) {
//РОСН, УГЗ, ИППК,ГИИ,Авиация в карточку Минского ГУМЧС/иных УМЧС не показывать
                if (($row->orgid != 8) && ($row->orgid != 9) && ($row->orgid != 10) && ($row->orgid != 11)&& ($row->orgid != 12)) {
                    if ($row->id_record == $recKey) {
                        ...
                        if ($row->state == 0) {
                            ...
                            if ($row->divizion_num == 0) {//ПАСО,УГЗ,ИППК,ГИИ
                                ?>
                ...
                foreach ($card as $row) {
                    ...
                    if (($row->orgid != 8) && ($row->orgid != 9)&& ($row->orgid != 10) && ($row->orgid != 11)&& ($row->orgid != 12)) {//РОСН, УГЗ, ИППК,ГИИ,Авиация в карточку Минского ГУМЧС/иных УМЧС не показывать
                        if ($row->id_record == $recKey) {
                            $y++;
                            ...
                            if ($y == 1) {
                                ...
                                if (($levelId == 1) && ($roleId == 3)) {//РЦУ admin
                                    ...
                                    <td rowspan="<?= $x ?>"><a href='/ss/edit/record/<?= $row->id_record ?>' target="_blank"> <span class='glyphicon glyphicon-pencil'></span></a></td>
                                    <td rowspan="<?= $x ?>"><a href='/ss/remove/<?= $row->id_record ?>' target="_blank"> <span class='glyphicon glyphicon-trash'></span></a></td>
                                            <?php
                                        }
                                    }
                                    ?>
